...
def reset_visual():
    ...
    val1 = slider.value()*4
    colors  = np.genfromtxt("vort_colors.txt", skip_header=i*len(faces), max_rows=len(faces), usecols = (val1, val1+1, val1+2, val1+3))
...
colors = np.genfromtxt("vort_colors.txt", max_rows=len(faces), usecols = (0, 1, 2, 3))
