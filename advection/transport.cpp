...
namespace advection
{
	...
	{
	//	for (int i = 0; i< size; i++) {for (int j = 0; j < size; j++) cout << matrix[i*size+j] << " "; cout << "\t" << right_hand_side[i] << endl;}
	//	cout << "==============================================" << endl;
		...
		for (int i=0; i<size; i++)
		{
			result[i] = x[i];//division by ten is absolute hack
	//		cout <<result[i] << " ";
		}
	//	cout <<  endl<< "==============================================" << endl;
	//	int counter = 0;
	...
	//	{
	//		double val=0.0;
	//		for (int j = 0; j < size; j++) val+=matrix[i*size+j]*result[j];
	//		cout << val << " ";
	//		if (abs(val-right_hand_side[i])<1e-4) counter++;
	//	} 
	//	cout << endl << endl << endl << counter << "/" << size << endl;
	...
	void advection_2d::calculate_velocities(Vector2 *& velocities, Vector2 *& velocities_original, double *& vorticities, double const & dt, const int& batch)
	{
		for (int i = 0; i < batch; i++)
		{
			...
			for (int j = 0; j < size; j++)
			{
				...
				for (int k = 0; k < 3; k++)
				{
					if (neighbor_ids[j*3+k] != -1)
					{
						...
						B += barycenter_dist_vector[j*3+k].x()*barycenter_dist_vector[j*3+k].y()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2);
						D += pow(barycenter_dist_vector[j*3+k].y()*1.0/barycenter_dist_vector[j*3+k].length(), 2);
						E += -barycenter_dist_vector[j*3+k].x()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2);
						F += -barycenter_dist_vector[j*3+k].y()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2);
					}
				}
				C = B;
				...
				for (int k = 0; k < 3; k++)
				{
					if (neighbor_ids[j*3+k] != -1)
					{
						...
						for (int l = 0; l < 3; l++)
						{
							...
							{
								A_1 += pow(barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].x()*1.0/barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].length(), 2);
								B_1 += barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].x()*barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].y()*pow(1.0/barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].length(), 2);
								D_1 += pow(barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].y()*1.0/barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].length(), 2);
								E_1 += -barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].x()*pow(1.0/barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].length(), 2);
								F_1 += -barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].y()*pow(1.0/barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].length(), 2);
							}
						}
						C_1 = B_1;
						double value1 = barycenter_dist_vector[j*3+k].x()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2);
						double value2 = barycenter_dist_vector[j*3+k].y()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2);
						...
						for (int l = 0; l < 3; l++)
						{
							if(neighbor_ids[neighbor_ids[j*3+k]*3+l] != -1)
							{
								double another_value1 = barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].x()*pow(1.0/barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].length(), 2);
								double another_value2 = barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].y()*pow(1.0/barycenter_dist_vector[neighbor_ids[j*3+k]*3+l].length(), 2);
			...
			//find_velos_from_stream_function(&velocities[i*size], result);
			...
			for (int j = 0; j < size; j++)
			{
				...
				{
					cout << result[j] << " " << result[neighbor_ids[j*3]] << " " << result[neighbor_ids[j*3+1]] << " " << result[neighbor_ids[j*3+2]] << endl;
				...
				{	
					...
					for (int k = 0; k < 3; k++)
					{
						if (neighbor_ids[j*3+k] != -1)
						{
							...
							B += barycenter_dist_vector[j*3+k].x()*barycenter_dist_vector[j*3+k].y()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2);
							D += pow(barycenter_dist_vector[j*3+k].y()*1.0/barycenter_dist_vector[j*3+k].length(), 2);
							E += barycenter_dist_vector[j*3+k].x()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2)*(x_vals[neighbor_ids[j*3+k]]-x_vals[j]);
							F += barycenter_dist_vector[j*3+k].y()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2)*(x_vals[neighbor_ids[j*3+k]]-x_vals[j]);
							E_2 += barycenter_dist_vector[j*3+k].x()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2)*(y_vals[neighbor_ids[j*3+k]]-y_vals[j]);
							F_2 += barycenter_dist_vector[j*3+k].y()*pow(1.0/barycenter_dist_vector[j*3+k].length(), 2)*(y_vals[neighbor_ids[j*3+k]]-y_vals[j]);
						}
					}
					C = B;
	...
	void advection_2d::calc_dynamic_arrays(vector<Triangle2*> triangles)
	{
		...
		barys.close();
		//cout << leftmost_x << " " << rightmost_x << " " << bottom_y << " " << top_y << endl;
		for (auto it(triangles.begin()); it != triangles.end(); it++)
		{
			is_boundary[k] = false;
			is_no_stick[k] = false;
